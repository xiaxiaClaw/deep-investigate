...
**第四步：搜集信息（必须批量并发抓取，-c 5）**

**严禁逐个抓取！禁止多次轮次抓取！**

操作流程（2步完成，不要多轮）：
1. 确定3-5个关键词，列出6-10个URL（知乎/百度/GitHub/微博等），**不要花时间找完美URL列表，差不多就抓**
2. **一次性批量并发抓取**：
   ```bash
   python fetch.py "URL1" "URL2" "URL3" "URL4" "URL5" "URL6" -o results.json -c 5
   ```
3. 批量完成后读取 results.json 分析，不要再反复抓取
...
- ✅ **找被忽略的信源**：除知乎/微博/百度外，还要找：GitHub Issues/Commit记录、学术论文/预印本、行业财报/招股书、GitHub Trending、冷门国外媒体——这些是多数人不会查但经常挖到真料的来源
