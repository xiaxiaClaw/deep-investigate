...
| 模式 | 适用场景 | 主会话状态 |
|------|---------|-----------|
| **直接调查** | 预计<20分钟的简单任务 | 阻塞等待结果 |
| **subagent 后台调查** | 预计>20分钟的复杂任务 | 不阻塞，可继续聊天 |

**建议**：拿不准的时候先问用户——"这个调查比较复杂，要不要我派一个专项小组去做？"
...
## 模式二：subagent 后台调查（复杂任务）

### 触发条件

遇到以下情况时，**必须**使用 subagent：
...
```
收到！这个调查比较复杂（预计需要 X 分钟），我派专项小组去做。
